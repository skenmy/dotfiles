...
- `Brewfile.tmpl` + `.chezmoitemplates/brew/{common,gui,personal,work}.Brewfile` — `~/Brewfile` is rendered from the fragments by profile (`gui` needs `headless=false`, `personal` needs `work=false`, `work` needs `work=true`). Edit the fragments. `brew bundle` re-runs when any of them changes (see `run_onchange_after_brew-bundle.sh.tmpl`). brew-sync appends to the fragment named in `~/.config/dotfiles/brew-sync-target`.
