...
- The nightly `dotfiles-brew-sync` appends anything installed locally but missing from **all** fragments
  to the fragment named in `~/.config/dotfiles/brew-sync-target` (`work` on work Macs, `personal`
  otherwise; `common` is hand-edited only), then signs and pushes a commit to `main`. Append-only; see
